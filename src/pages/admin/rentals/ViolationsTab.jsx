...
import { Shield, Eye, User, Car, MapPin, Calendar, DollarSign, FileText, Phone, Mail, Search } from 'lucide-react';
...
const ViolationsTab = () => {
  ...
  // Filtered violations for search
  const filteredViolations = violations.filter(v =>
    v.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    v.rental.renter.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    v.rental.vehicle.licensePlate?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <>
      {/* Search input outside the card, styled like RentalsTab */}
      <div className='flex items-center gap-4 mb-4'>
        <div className='relative flex-1 max-w-sm'>
          <Search className='absolute left-2 top-2.5 h-4 w-4 text-muted-foreground' />
          <Input
            placeholder='Tìm kiếm theo tên vi phạm...'
            value={searchTerm}
            ...
            {selectedViolation && (
              <div className='grid gap-6 py-4'>
                ...
                <div className='bg-yellow-50 p-4 rounded-lg border border-yellow-200'>
                  ...
                  {/* Chi tiết các loại tiền và phí vi phạm */}
                  <div className='grid grid-cols-4 gap-4 mb-4'>
                    <div>
                      <Label className='text-sm font-medium text-muted-foreground'>Chi phí thuê</Label>
                      <p className='text-lg font-bold text-blue-600'>
                        {formatCurrency(selectedViolation.rental.rentalCost)}
                      </p>
                    </div>
                    <div>
                      <Label className='text-sm font-medium text-muted-foreground'>Bảo hiểm</Label>
                      <p className='text-lg font-bold text-purple-600'>
                        {selectedViolation.rental.insurance > 0 ? formatCurrency(selectedViolation.rental.insurance) : '0 ₫'}
                      </p>
                    </div>
                    <div>
                      <Label className='text-sm font-medium text-muted-foreground'>Tiền cọc</Label>
                      <p className='text-lg font-bold text-orange-600'>
                        {selectedViolation.rental.depositAmount ? formatCurrency(selectedViolation.rental.depositAmount) : '0 ₫'}
                      </p>
                    </div>
                    <div>
                  ...
                  {/* Tổng thanh toán ở góc phải */}
                  <div className='flex justify-end'>
                    <div className='bg-white p-4 rounded-lg border-2 border-yellow-300 shadow-sm min-w-[220px]'>
                      <Label className='text-sm font-medium text-muted-foreground block text-center'>Tổng thanh toán</Label>
                      <p className='text-3xl font-bold text-green-700 text-center mt-1'>
                        {formatCurrency((selectedViolation.rental.totalCost || 0))}
                      </p>
                    </div>
                  </div>
                </div>
